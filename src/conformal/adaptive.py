...
def out_of_fold_predict(
    ...
    吸収されて区間は変わらない（卒論 注意 rem:normalized-reduction。sigma を定数倍しても
    区間が同じことは tests/test_adaptive.py で検査している）。問題になるのは、縮み方が x によって
...
class SigmaEstimator:
    """sigma(x) の推定器。predict(X) が正の尺度を返す（卒論 sec:normalized-score、eq:sigma-hat）。
