...
def normalized_residual(
    ...
    r"""正規化残差スコア  S = |y - \hat{f}(x)| / \hat{\sigma}(x)（卒論 sec:normalized-score、eq:score-normalized）。
...
def cqr_score(y: np.ndarray, q_lo: np.ndarray, q_hi: np.ndarray) -> np.ndarray:
    r"""CQR の非適合度スコア（卒論 sec:cqr、eq:cqr-score。Romano et al. 2019）。
...
def cqr_interval(
    ...
    r"""CQR の予測区間（卒論 sec:cqr、eq:cqr-interval）。
