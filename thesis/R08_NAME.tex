...
\begin{example}[不均一分散と幅一定の区間]
  ...
  \textbf{意味.} 幅 $2q$ が $x$ によらず一定なので、$\sigma(x)$ が小さい領域では条件付き被覆が
  高く、大きい領域では低い。周辺被覆 $\Prob(Y\in C(X))=\Exp[F_{|\varepsilon|}(q/\sigma(X))]$
  はこれらを $P_X$ で平均した値であり、較正によって $1-\alpha$ 以上に保たれる。したがって、
  周辺被覆が $1-\alpha$ を大きく超えていない限り、\textbf{$\sigma$ の大きい側では条件付き被覆が
  $1-\alpha$ を下回る}。実際、条件付き被覆が $x$ について一定でなく周辺被覆がちょうど
  $1-\alpha$ なら、$1-\alpha$ を下回る $x$ が正の $P_X$-確率で存在し（平均に等しい定数でない
  関数は、平均を下回る点を正の確率でもつ）、単調性よりそれは $\sigma(x)$ の大きい側にある。
  周辺被覆は領域ごとの過不足が打ち消し合った結果にすぎない。
