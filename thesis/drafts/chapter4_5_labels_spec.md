# 第4・5章 ラベル仕様書（v1.2：第5章分のみ）

> **この文書について（TeX 本文には含めない）**
> - 第4章（拡張）と第5章（数値実験）の本文はまだ書いていない。この文書は、コードの docstring と
>   テストが先に参照している式ラベルを確定するためのもの（v1 では 6 つ。第 4 章分は移行済み）。第4・5章の仕様書を書くときは
...
>   第 4 章分の 4 ラベルは `chapter4_sections_spec.md` と TeX に移行済み（v1.2。TeX の `\label` が正典）。
>   第 5 章分（`eq:coverage-rate`, `eq:mean-width`）は未統合。
...
| v1.2 | 第 4 章分 4 ラベルの行を削除（TeX に `\label` が付いたため。コミット `fee3bfe`） |
